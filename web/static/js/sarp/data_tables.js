...
async function cargarVuelos(pagina = 1) {
  ...
  data.vuelos.forEach((vuelo) => {
      ...
      let fila = `<tr>
          ...
          <td>
              <button class="btn editar_unidad" data-unidad="${vuelo.id_vuelo}">✏️</button>
              <button class="btn generar-excel" data-unidad="${vuelo.id_vuelo}">📄</button>
              <button onclick="eliminarVuelo('${vuelo.id_vuelo}')" class="btn eliminar_unidad">🗑️</button>
          </td>
      </tr>`;
