async function fetchWithLoader(url, options = {}) {
  let activeRequests = 0; // Ejemplo: definir si no está global
  activeRequests++;
  showLoader();
  ...
  try {
    ...
    if (!response.ok) {
      // Leer el cuerpo del error para más detalles si la respuesta no es OK
      const errorBody = await response.text();
      throw new Error(
        `Error en la solicitud: ${response.status} ${response.statusText} - ${errorBody}`
      );
    }
    return await response.json(); // Se asume que /api/buscar_vuelo/ devuelve JSON
  } catch (error) {
    console.error("Error en fetchWithLoader:", error);
    throw error; // Propaga el error para que `buscarVueloPorId` lo maneje
  } finally {
    activeRequests--;
    hideLoader();
  }
...
document.addEventListener("DOMContentLoaded", function () {
  document
    .getElementById("filterJerarquia") // Asegúrate de que este ID sea correcto para tu input de búsqueda
    .addEventListener("keydown", function (event) {
      if (event.key === "Enter") {
        const valor = this.value.trim();
        if (valor === "") {
          cargarVuelos(1); // Si se borra el texto, recarga todo (asume que cargarVuelos existe)
        } else {
...
async function buscarVueloPorId(id) {
  try {
    // 🚀 Construye la URL completa usando la variable de Django y reemplazando el marcador.
    const urlCompleta = BUSCAR_VUELO_BASE_URL_PLACEHOLDER.replace("0000", id);

    // console.log("Buscando vuelo en URL:", urlCompleta); // Para depuración

    let response = await fetchWithLoader(urlCompleta);

    // Tu endpoint `/api/buscar_vuelo/${id}` debe devolver un array con un único objeto,
    // o un objeto directamente. Aquí asumo que devuelve un array y tomas el primer elemento.
    let vuelo = response[0];
    ...
    let fila = `<tr>
                ...
                <td>${vuelo.id_operador__jerarquia} ${
      vuelo.id_operador__nombres
    } ${vuelo.id_operador__apellidos}</td>
                <td>
                    ${
                ...
                <td>
                    <button class="btn editar_unidad" data-unidad="${
                      vuelo.id_vuelo
                    }">
                    ...
                    <button class="btn generar-excel" data-unidad="${
                      vuelo.id_vuelo
                    }">
