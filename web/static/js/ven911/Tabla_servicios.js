...
function cargarTablaServicios() {
    fetch(api_table_911)
        ...
        .then(data => {
            ...
            data.servicios.forEach((servicio, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${servicio.municipio?.nombre || 'N/A'}</td>
                    <td>${servicio.parroquia?.nombre || 'N/A'}</td>
                    <td>${servicio.tipo_servicio?.nombre || 'N/A'}</td>
                    <td>${servicio.unidad?.nombre || 'N/A'}</td>
                    <td>${servicio.jefe_de_comision?.nombre_completo || 'N/A'}</td>
                    <td>
                        <button class="btn" data-id="${servicio.id}">
        ...
        .catch(error => {
            ...
            tbody.innerHTML = `<tr><td colspan="10" class="text-center text-danger">Error al cargar los datos: ${error.message}</td></tr>`;
        });
}
