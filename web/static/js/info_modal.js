...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (tbodyContainer) {
    tbodyContainer.addEventListener("click", async function (event) {
      // Verificar si se hizo clic en un botón con la clase "button-info"
      if (event.target && event.target.matches(".button-info")) {
        info_modal.textContent = ""; // Limpiar contenido del modal

        const id = event.target.getAttribute("data-id");
        const id_tipo_procedimiento = event.target.getAttribute("data-id_procedimiento");
