...
const user = document.getElementById("user-login").textContent; // Obtener el usuario actual
...
async function cargarProcedimientos(fecha) {
    const url = `/serviciosmedicos?fecha=${fecha}`; // URL con la fecha como parámetro
      ...
      const data = await fetchWithLoader(url, {
        ...
        procedimientos.forEach(proc => {
            ...
            const fields = [
                ...
                { content: proc.tipo_servicio },  // Tipo de servicio
                { content: proc.solicitante_externo },  // Tipo de servicio
                ...
                { content: proc.id_tipo_procedimiento__tipo_procedimiento, className: 'fixed-width' },  // Dirección
                { content: `<td>
              ${user === "Sala_Situacional" || user === "SeRvEr" ?
                  `<button
                    type="button"
                    class="button_delete"
                    data-id="${proc.id}"
